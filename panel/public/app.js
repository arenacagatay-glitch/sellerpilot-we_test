...
function renderDrafts() {
  ...
  wrap.innerHTML = state.drafts.map((d, i) => `
    <div class="draft" data-id="${d.id}">
      <h4>#${i + 1} · ${esc(d.title)} <span class="muted">(${esc(d.barcode)})</span>${d.setSize ? ` <span class="tag ok">${d.setSize}'li set</span>` : ''}${d.matchType === 'akilli' ? ' <span class="tag ok">🧠 Akıllı eşleşme</span>' : ''}${d.matchType === 'rutin-hediye' ? ' <span class="tag ok">🎁 Rutin/Hediye</span>' : ''}${d.copySource ? ` <span class="tag ${d.copySource === 'openai' ? 'ok' : 'no'}">${d.copySource === 'openai' ? '🤖 AI metni' : '⚙️ Otomatik metin'}</span>` : ''}</h4>
      <div class="grid2">
...
$('#comboSuggest').addEventListener('click', async () => {
  const q = $('#comboKeyword').value.trim(); // bos = tum kategoriler
  ...
  $('#comboStatus').textContent = '⏳ Ürünler taranıyor, başlık/açıklama yazılıyor (tüm katalogda biraz sürebilir)...';
  try {
    ...
    if (!data.combos.length) {
      $('#comboStatus').textContent = `⚠️ Uygun ürün bulunamadı (${data.candidates} aday tarandı, stoklu+onaylı olmalı, en az 2 gerekli).`;
      return;
    }
    ...
    data.combos.forEach((c) => {
      bySize[c.setSize] = (bySize[c.setSize] || 0) + 1;
      byType[c.matchType] = (byType[c.matchType] || 0) + 1;
    });
